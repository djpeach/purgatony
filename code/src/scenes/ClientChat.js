...
export default class ChatScene extends Phaser.Scene {
  ...
  setupUiElements() {
    ...
    this.add.text(this.box.getTopLeft().x + 16, this.box.getTopLeft().y + 16, this.chat.prompt, { fontSize: '32px', color: '#000' });
    ...
    this.resultText = this.add.text(this.box.getTopLeft().x + 16, this.box.getBottomLeft().y - 30, this.chat.results[this.chat.results.length - 1].prompt, { fontSize: '28px', color: '#000' });
    this.chat.options.forEach((option, i) => {
    ...
    this.chat.options[0].selected = true;
  }

  setupEvents() {
    this.cursors.down.on('down', () => {
      for (const [i, option] of this.chat.options.entries()) {
        if (option.selected) {
          option.selected = false;
          this.chat.options[i === this.chat.options.length - 1 ? 0 : i + 1].selected = true;
          break
        }
      }
    });
    ...
    this.cursors.up.on('down', () => {
      for (const [i, option] of this.chat.options.entries()) {
        if (option.selected) {
          option.selected = false;
          this.chat.options[i === 0 ? this.chat.options.length - 1 : i - 1].selected = true;
          break
        }
      }
    });
    ...
    this.cursors.b_2.on('down', () => {
      for (const [i, option] of this.chat.options.entries()) {
        if (option.selected) {
          this.resultText.setText(this.chat.results[i].prompt);
          break;
        }
      }
    });
  ...
  update(time, delta) {
    ...
    this.chat.options.forEach((option) => {
      if (option.selected) {
